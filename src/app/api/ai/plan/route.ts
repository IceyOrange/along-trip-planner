...
【路线排序原则——关键】
waypoints 的顺序不要严格按照对话提及的先后顺序排列，而应根据地理位置的分布来智能排序：
- 将同一区域或相邻区域的地点安排在一起，避免路线来回折返
- 考虑城市地理布局（如市中心、东区、西区、南区、北区），按合理的游览顺序排列
- 尽量选择距离相近的地点作为相邻 waypoints，减少总体移动距离
- 如果用户没有指定顺序，默认按照地理位置最优路径排列，而不是对话出现顺序

【交通方式建议】
为每个 waypoint 设置合适的 recommendedTransport：
- walking: 相邻地点在同一商圈/景区/步行街区，距离很近
- transit: 市内不同区域之间，距离中等
- driving: 跨区、远郊或距离较远
